...
(function() {
  ...
  function init() {
    ...
    window.parent.postMessage({ type: PREFIX + 'ready' }, PARENT_ORIGIN);
  }

  function createOverlay() {
    overlayEl = document.createElement('div');
    overlayEl.id = '__og-inspector-overlay';
  ...
  function createTooltip() {
    ...
    tooltipEl.id = '__og-inspector-tooltip';
  ...
  function onMessage(e) {
    // PARENT_ORIGIN === '*' is the documented last-resort fallback in
    // WorkerGateway.extractParentOrigin (used when the WS handshake reaches
    // the worker stripped of its browser Origin/Referer — typical when the
    // og-ui talks to the worker through the og-manager bridge). Without this
    // wildcard escape, the literal comparison `e.origin !== "*"` is always
    // true and the script silently drops every postMessage from the parent
    // (activate/deactivate/ping), even though it can still send 'ready' back
    // because postMessage with targetOrigin='*' is permitted as a destination.
    // This is the same defense the screenshot script already implements.
    if (PARENT_ORIGIN !== '*' && e.origin !== PARENT_ORIGIN) return;
    var data = e.data;
    if (!data || !data.type || data.type.indexOf(PREFIX) !== 0) return;
  ...
  function onClick(e) {
    ...
    var ref = captureElementRef(el);
    selectedElements.push(ref);
    markSelected(el);

    window.parent.postMessage({
      type: PREFIX + 'element-selected',
      data: ref
    }, PARENT_ORIGIN);
  }
  ...
  function onKeydown(e) {
    if (e.key === 'Escape') {
      deactivate();
      window.parent.postMessage({ type: PREFIX + 'deactivated' }, PARENT_ORIGIN);
    }
  }

  function isInspectorElement(el) {
    if (!el || !el.id) return false;
    return el.id.indexOf('__og-inspector') === 0;
  }
  ...
  function showTooltip(el, rect) {
    ...
    if (text && text.length > 30) text = text.slice(0, 30) + '…';
    if (text) label += ' "' + text + '"';
  ...
  function markSelected(el) {
    ...
    el.dataset.ogInspectorSelected = 'true';
  }

  function removeSelectedMarkers() {
    var marked = document.querySelectorAll('[data-og-inspector-selected]');
    for (var i = 0; i < marked.length; i++) {
      marked[i].style.outline = '';
      marked[i].style.outlineOffset = '';
      delete marked[i].dataset.ogInspectorSelected;
    }
  }

  function captureElementRef(el) {
    var rect = el.getBoundingClientRect();
    ...
    return {
      ...
      classes: el.className && typeof el.className === 'string'
        ...
        : [],
      textContent: truncate((el.textContent || '').trim(), 80),
  ...
  function buildSelector(el) {
    ...
    while (current && current !== document.body && current !== document.documentElement) {
      ...
      if (current.className && typeof current.className === 'string') {
        var cls = current.className.trim().split(/\s+/)
          .filter(function(c) { return c && c.indexOf('__og-inspector') === -1; })
          .slice(0, 2);
  ...
  function getRelevantAttributes(el) {
    var attrs = {};
    var names = ['type', 'role', 'aria-label', 'href', 'src', 'alt', 'name',
                 'placeholder', 'data-testid', 'data-cy', 'action', 'method'];
    ...
    for (var j = 0; j < all.length; j++) {
      if (all[j].name.indexOf('data-') === 0 && !attrs[all[j].name]) {
        attrs[all[j].name] = all[j].value;
      }
    }
    return attrs;
  }
  ...
  window.__ogInspectorCleanup = cleanup;
  init();
})();
;
(function() {
  ...
  var PARENT_ORIGIN = "*";
  var PREFIX = 'og-screenshot:';

  // Output bounds — keep thumbnails visually useful but small on the wire.
  // 800x600 at JPEG q=0.85 typically lands between 30 KB and 150 KB.
  var MAX_WIDTH = 800;
  var MAX_HEIGHT = 600;
  ...
  function onMessage(e) {
    if (e.origin !== PARENT_ORIGIN && PARENT_ORIGIN !== '*') return;
    var data = e.data;
    if (!data || data.type !== PREFIX + 'capture') return;
    captureScreenshot();
  }

  function captureScreenshot() {
    if (!window.htmlToImage || typeof window.htmlToImage.toCanvas !== 'function') {
      sendError('html-to-image not available (UMD bundle missing)');
      return;
    }

    var target = resolveTarget();
    if (!target) {
      sendError('No capture target found (#app, #root, main, body all missing)');
      return;
    }

    // cacheBust: false — the browser cache already holds the freshly-loaded
    // iframe assets; re-fetching each one with a timestamp on every capture
    // turns a ~2s job into 10-20s and causes the parent-side 20s timeout to
    // trigger, making subsequent captures appear to "not update" (the WS
    // emit never fires because we resolve(null) on timeout).
    window.htmlToImage
      .toCanvas(target, {
        pixelRatio: 1,
        cacheBust: false,
      ...
      .then(function(sourceCanvas) {
        try {
          var dataUrl = downscaleToJpeg(sourceCanvas);
          sendResult(dataUrl);
        } catch (err) {
          sendError('Downscale failed: ' + (err && err.message ? err.message : String(err)));
        }
      })
      .catch(function(err) {
        sendError('html-to-image failed: ' + (err && err.message ? err.message : String(err)));
      });
  }

  // Re-draws the source canvas onto a smaller canvas that preserves aspect
  // ratio and fits within MAX_WIDTH x MAX_HEIGHT. Returning JPEG (not PNG)
  // divides the base64 payload size by roughly 5-20x for photographic or UI
  // content, which is what keeps us under Socket.IO's 1 MB frame limit.
  function downscaleToJpeg(sourceCanvas) {
    var srcW = sourceCanvas.width || 1;
    var srcH = sourceCanvas.height || 1;

    var scale = Math.min(MAX_WIDTH / srcW, MAX_HEIGHT / srcH, 1);
    ...
    if (!ctx) throw new Error('Could not get 2d context for downscale canvas');

    // Fill white so any transparent pixels flatten correctly in JPEG (which
    // has no alpha channel and would otherwise render them as black).
  ...
  function sendResult(base64) {
    window.parent.postMessage({ type: PREFIX + 'result', data: base64 }, PARENT_ORIGIN);
  }

  function sendError(message) {
    window.parent.postMessage({ type: PREFIX + 'error', error: message }, PARENT_ORIGIN);
  }
  ...
  window.__ogScreenshotCleanup = cleanup;
  window.addEventListener('message', onMessage);
})();
;
